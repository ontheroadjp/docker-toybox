...
out="/etc/mysql/conf.d/mariadb.cnf"
sed -i -e "s:^#default-character-set *= *utf8:default-character-set = utf8:" ${out}
sed -i -e "s:^#character-set-server *= *utf8:character-set-server = utf8:" ${out}
sed -i -e "s:^#collation-server *= *utf8_general_ci:collation-server = utf8_general_ci:" ${out}
sed -i -e "s:^#character_set_server *= *utf8:character_set_server = utf8:" ${out}
sed -i -e "s:^#collation_server *= *utf8_general_ci:collation_server = utf8_general_ci:" ${out}
...
EOF
    ...
    out="/docker-entrypoint-initdb.d/change_master_to.sh"
